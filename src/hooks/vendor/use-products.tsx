...
export const useProductMutations = (shopId: string) => {
  ...
  const invalidateProducts = () => {
    ...
    // Old fix (superseded): the products DataTable used ["datatable", ...] as its
    // cache key, so we invalidated that prefix too. The table's cache key is now
    // prefixed with vendorProductsKeys.lists() (see createVendorProductsFetcher in
    // use-vendor-entity-fetcher.ts), so the invalidation above catches it.
