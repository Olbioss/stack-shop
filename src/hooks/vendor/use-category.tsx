...
export const useCategoryMutations = (shopId: string) => {
  ...
  const invalidateCategories = () => {
    ...
    // Old fix (superseded): the categories DataTable used ["datatable", "shop", params]
    // as its cache key, so we had to invalidate that prefix too. Now the table's cache
    // key is prefixed with ["vendor", "categories", shopId] (see
    // createVendorCategoriesFetcher in use-vendor-entity-fetcher.ts), so the
    // invalidation above catches it automatically.
    // queryClient.invalidateQueries({ queryKey: ["datatable", "shop"] });
  };
