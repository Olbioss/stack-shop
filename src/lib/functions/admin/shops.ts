...
import { adminShopsQuerySchema } from "#/lib/validators/admin/shop-query";
...
export const getAdminShops = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<AdminShopListResponse> => {
    ...
    const [countResult, shopList] = await Promise.all([
      ...
      db
        .select({ shop: shops, vendor: vendors, owner: user })
        .from(shops)
    ...
    if (shopList.length === 0) return { data: [], total, limit, offset };
