...
interface EntityFormDialogExtendedProps<T extends Record<string, any>> {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: T) => void | Promise<void>;
  isSubmitting?: boolean;
  initialValues?: Partial<T> | null;
  /** Extra default values merged in for form fields that aren't top-level `fields` entries (e.g. names used only inside `custom` renders). */
...
export function EntityFormDialogExtended<T extends Record<string, any>>({
  open,
  onOpenChange,
  onSubmit,
  ...
  contentClassName,
  scrollable = true,
}: EntityFormDialogExtendedProps<T>) {
            ...
            <UIField>
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  const parsed =
                    field.step !== undefined && Number.isInteger(field.step)
                      ? parseInt(raw, 10)
                      : parseFloat(raw);
                ...
                {options.map((option) => (
                  <div key={option.value} className="flex items-center gap-2">
                    <RadioGroupItem
                      ...
                      value={option.value}
                    />
                    <Label htmlFor={`${fieldState.name}-${option.value}`}>
                      {option.icon && (
                        <option.icon className="inline-block mr-1.5 size-4" />
                      )}
                      {option.label}
                    </Label>
                  </div>
