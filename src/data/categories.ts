...
const buildCategoryTree = (categories: Category[]): CategoryWithChildren[] => {
  ...
  const attach = (parentId: string | undefined): CategoryWithChildren[] =>
    (byParent.get(parentId) ?? [])
      .slice()
      .sort((a, b) => a.sortOrder - b.sortOrder)
      .map((c) => ({ ...c, subcategories: attach(c.id) }));

  return attach(undefined);
};

export const categoryTree: CategoryWithChildren[] =
  buildCategoryTree(mockCategories);
